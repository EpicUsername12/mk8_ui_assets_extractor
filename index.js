...
Object.keys(languages).forEach((language) => {
  ...
  if (!fs.existsSync("assets/" + language)) {
    fs.mkdirSync("assets/" + language);
  }

  fs.writeFileSync(
    `assets/${language}/characters.json`,
    JSON.stringify(lang_chara, undefined, 4)
  );

  fs.writeFileSync(
    `assets/${language}/body_karts.json`,
    JSON.stringify(lang_body, undefined, 4)
  );

  fs.writeFileSync(
    `assets/${language}/tire_karts.json`,
    JSON.stringify(lang_tire, undefined, 4)
  );
});
